...
func (t *tunAutoRedirect) setupIPTablesPreRouting(iptablesPath string) error {
	...
	if len(t.tunOptions.IncludeInterface) > 0 {
		includeCounter++
	}
	if len(t.tunOptions.IncludeUID) > 0 {
		includeCounter++
	}
	if includeCounter > 1 {
		return E.New("only one of `*_include_address`, `include_interface`, `include_uid` can be specified")
	}
	...
	} else if len(t.tunOptions.IncludeInterface) > 0 {
	...
	} else if len(t.tunOptions.IncludeUID) > 0 {
